...
# Uso: bash audit.sh dominio.cl

set -euo pipefail

TARGET="${1:-}"
if [[ -z "$TARGET" ]]; then
    echo "[ERROR] Especifica un dominio: bash audit.sh dominio.cl"
    exit 1
fi
...
echo "[→] Iniciando escaneo OSINT: ${TARGET}"
...
checkdmarc "${TARGET}" -f json \
    ...
    echo "[✓] checkdmarc completado" || \
    echo "[!] checkdmarc: revisar resultados"

# 2. SUBFINDER — subdominios
echo "[*] subfinder..."
subfinder -d "${TARGET}" -all -silent \
    -o "${BASE}/subdomains/subfinder_${TARGET}.txt" 2>/dev/null && \
    echo "[✓] subfinder: $(wc -l < ${BASE}/subdomains/subfinder_${TARGET}.txt) subdominios" || \
    echo "[!] subfinder: sin resultados"
...
if [[ -s "${BASE}/subdomains/subfinder_${TARGET}.txt" ]]; then
    ...
    /usr/local/bin/httpx \
        ...
        echo "[✓] httpx: $(wc -l < ${BASE}/http/httpx_${TARGET}.json) hosts activos" || \
        echo "[!] httpx: sin resultados"
else
    echo "[!] Sin subdominios, omitiendo httpx"
fi
...
if [[ -s "${BASE}/http/httpx_${TARGET}.json" ]]; then
    ...
    nuclei \
        ...
        echo "[!] nuclei: sin resultados"
else
    echo "[!] Sin hosts activos, omitiendo nuclei"
fi
...
if [[ "$FULL" == "--full" ]] && [[ -s "${BASE}/http/httpx_${TARGET}.json" ]]; then
    ...
    if [[ -s "${BASE}/nmap/live_hosts.txt" ]]; then
        nmap -sV -sC --open -T4 \
            ...
            2>/dev/null && \
            echo "[✓] nmap completado" || \
            echo "[!] nmap: sin resultados o error"
    else
        echo "[!] Sin hosts para nmap"
    fi
fi

echo ""
echo "[✓] Escaneo completado: ${BASE}"
